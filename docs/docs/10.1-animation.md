...
By default, React uses the transitionend event (https://developer.mozilla.org/en-US/docs/Web/Events/transitionend) to clean-up DOM elements once their animation completes. This event may not fire for a range of reasons (including the browser tab being de-focused before the animation ends). In the worst case, a missed transitionend event can lead to `leaving` children remaining in the DOM indefinitley.

React provides an alternative way to clean up animated elements: specify the precise expected duration of your animation using the `transitionLeaveTimeout`, `transitionEnterTimeout` and `transitionAppearTimeout` props on `ReactCSSTransitionGroup`. This may require duplication of transition times between your CSS and component code, but it can also provide a stability improvement.

If you're having problems -- for example, if you're seeing console warning messages like this one: `transition(): tried to perform an animation without an animationend or transitionend event after (5000ms). You should either disable this transition in JS, add a CSS animation/transition or specify a timeout for your animation.` -- then we recommend specifying these times.

Note: if a transitionend event is detected before the specified timeout has elapsed, then React will clean-up the animation immediately. This means that it's OK to specify a 'failsafe' timeout that's longer than the actual animation - the clean-up will usually be triggered immediately by the transitionend event. Specifying a timeout that's too short will curtail the animation.