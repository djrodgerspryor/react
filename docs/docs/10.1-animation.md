...
React uses the transitionend event (https://developer.mozilla.org/en-US/docs/Web/Events/transitionend) to clean-up DOM elements once their animation completes. This event may not fire for a range of reasons (including the browser tab being de-focused before the animation ends); react will automatically clean up any un-ended animations after `5000ms` under the assumption that the event will never be sent. We recommend setting the precise expected duration of your animation using the `transitionLeaveTimeout`, `transitionEnterTimeout` and `transitionAppearTimeout` props on `ReactCSSTransitionGroup` so that it can be cleaned-up immediately, even if the transitionend event doesn't fire.